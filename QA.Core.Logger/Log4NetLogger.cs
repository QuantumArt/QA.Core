...
namespace QA.Core.Logger
{
    ...
    public class Log4NetLogger : ILogger
    {
        ...
        public Log4NetLogger(string fileName, string loggerName)
        {
            ...
            log4net.Config.XmlConfigurator.Configure(repo, new FileInfo(fileName));
            Logger = log4net.LogManager.GetLogger(assembly, loggerName);
        }
        ...
        public virtual void ErrorException(string message, Exception exception, params object[] parameters)
        {
            try
            {
                ...
                {
                    Logger.Error($"{ex.Message}{Environment.NewLine}{ex.StackTrace}{Environment.NewLine}{ex.Source}",
                        ex);
                }
            }
        ...
        /// <param name="parameters">Дополнительные параметы</param>
        [Obsolete(
            "Необходимо использовать метод с сигнатурой \"void Info(Func<string> message, params object[] parameters)\"")]
        ...
        /// <param name="parameters">Дополнительные параметы</param>
        [Obsolete(
            "Необходимо использовать метод с сигнатурой \"void Debug(Func<string> message, params object[] parameters)\"")]
        ...
        /// <param name="parameters">Дополнительные параметы</param>
        [Obsolete(
            "Необходимо использовать метод с сигнатурой \"void Error(Func<string> message, params object[] parameters)\"")]
        ...
        public virtual void Fatal(string message, Exception exception, params object[] parameters)
        {
            try
            {
                ...
                foreach (var ex in exception.Flat())
                {
                    Logger.Fatal($"{ex.Message}{Environment.NewLine}{ex.StackTrace}{Environment.NewLine}{ex.Source}",
                        ex);
                }
            }
            catch (Exception ex)
        ...
        /// <param name="parameters">Дополнительные параметы</param>
        [Obsolete(
            "Необходимо использовать метод с сигнатурой \"void Fatal(Func<string> message, params object[] parameters)\"")]
        ...
        /// <param name="eventLevel">Требуемый уровень критичности сообщения</param>
        public void Log(Func<string> message, Action<IDictionary<object, object>> propertiesSetter,
